...
export default function AppCard({ app, onUpdate, onDelete }) {
  ...
  const handleMarkAsUpdated = async (e) => {
    e.stopPropagation()
    try {
      await appsApi.markAsUpdated(app.id)
      toast.success(`${app.name} marqué comme mis à jour`)
      onUpdate()
    } catch (error) {
      toast.error('Erreur lors de la mise à jour du statut')
      console.error('Mark as updated error:', error)
    }
  }
    ...
    >
      ...
      {showActions && (
        <div className="absolute top-4 right-4 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {app.hasUpdate && app.latest_version !== app.ignore_version && (
            <button
